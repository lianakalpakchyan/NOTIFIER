...
class BaseController(metaclass=ABCMeta):
    ...
    def slack_service(self) -> SlackService:
        if self._slack_service is None:
            ...
            self._slack_service = SlackService(self.logger_provider, api_token)
...
class APIController(BaseController):
    def process_event(self, event_entity):
        ...
        use_case.execute()
        abs_path = os.path.abspath('.')[:os.path.abspath('.').find('NOTIFIER') + 8]
        file_name = os.path.join(abs_path, 'src', 'logs', 'notifier.log')
        log_path = file_name
        with open(log_path) as app_logs:
            last_line = app_logs.readlines()[-1]
            log = last_line.split('::')[-1]
