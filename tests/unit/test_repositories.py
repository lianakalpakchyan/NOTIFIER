...
class ConfigRepository(TestCase):
    ...
    def test_get_one_should_return_EnvItemEntity_slack_channel_id(self):
        slack_api_token_entity = self.repo.get_one('SLACK_CHANNEL_ID')

        test_slack_api_token_entity = EnvItemEntity('SLACK_CHANNEL_ID', os.getenv('SLACK_CHANNEL_ID'))
        self.assertEqual(slack_api_token_entity.key, test_slack_api_token_entity.key)
        self.assertEqual(slack_api_token_entity.value, test_slack_api_token_entity.value)
